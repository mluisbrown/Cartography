...
/// - parameter block:   A block that declares the layout for `views`.
///
@discardableResult public func constrain<T: Hashable>(_ views: [T: View], replace group: ConstraintGroup = ConstraintGroup(), block: @noescape ([T : LayoutProxy]) -> ()) -> ConstraintGroup {
    let context = Context()
